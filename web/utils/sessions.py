...
def _init_session_state() -> None:
    ...
    for n in config.FeatureType:
        if n.value not in st.session_state[SESSION_KEY_NAME_DOCQ][SessionKeySubName.CHAT.value]:
            st.session_state[SESSION_KEY_NAME_DOCQ][SessionKeySubName.CHAT.value][n.value] = {}


def _get_session_value(name: SessionKeySubName, key_: str = None, subkey_: str = None) -> Any | None:
    _init_session_state()
    val = st.session_state[SESSION_KEY_NAME_DOCQ][name.value]
...
def _set_session_value(val: Any | None, name: SessionKeySubName, key: str = None, subkey: str = None) -> None:
    ...
    if key is None and subkey is None:
        ...
        st.session_state[SESSION_KEY_NAME_DOCQ][name.value][key] = val
    else:
        st.session_state[SESSION_KEY_NAME_DOCQ][name.value][key][subkey] = val
...
def get_chat_session(type_: config.FeatureType = None, key_: SessionKeyNameForChat = None) -> Any | None:
    ...
    return _get_session_value(
        ...
        type_.value if type_ is not None else None,
        key_.value if key_ is not None else None,
    )
...
def get_authenticated_user_id() -> int | None:
    ...
    return _get_session_value(SessionKeySubName.AUTH, SessionKeyNameForAuth.ID.value)


def get_settings_session(key: SessionKeyNameForSettings = None) -> dict | None:
    """Get the settings session value."""
    return _get_session_value(SessionKeySubName.SETTINGS, key.value if key is not None else None)


def set_settings_session(val: dict = None, key: SessionKeyNameForSettings = None) -> None:
    """Set the settings session value."""
    _set_session_value(val, SessionKeySubName.SETTINGS, key.value if key is not None else None)
