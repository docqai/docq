...
from typing import Any, Callable, Dict, List, Literal, Optional, Self, Tuple, Type, Union
...
class ConversableAgent(Agent):
    ...
    @tracer.start_as_current_span("ConversableAgent.__init__")
    def __init__(
        self: Self,
        name: str,
    ...
    @tracer.start_as_current_span("ConversableAgent.register_reply")
    def register_reply(
        self: Self,
    ...
    @tracer.start_as_current_span("ConversableAgent.update_system_message")
    ...
    @tracer.start_as_current_span("ConversableAgent.update_max_consecutive_auto_reply")
    ...
    @property
    def use_docker(self: Self) -> Union[bool, str, None]:
    ...
    def _append_to_log(self: Self, event: Literal["message_received","message_sent", "code_executed", "reply_function_called"], message: dict | str ) -> None:
        """Append a message to the agent log.

        Args:
            message (str): message to be appended to the agent log.
            agent (Agent): the agent.
        """
        dt = datetime.utcnow()
        m = message
        if isinstance(m, dict):  # noqa: SIM102
            if event == "message_received":
                m["recipient_agent"] = self.name
            elif event == "message_sent":
                m["sender_agent"] = self.name
        self._agent_log.append({"timestamp": str(dt), "event": event, "message": message})

    @tracer.start_as_current_span("ConversableAgent._append_oai_message")
    def _append_oai_message(self, message: Union[Dict, str], role, conversation_id: Agent) -> bool:
        ...
        span.set_attribute("message.raw", json.dumps(message))
        span.set_attribute("conversation_id", conversation_id.name)
        span.set_attribute("self.agent.name", self.name)
        span.set_attribute("agent_type", self.__class__.__name__)
        # create oai message to be appended to the oai conversation that can be passed to oai directly.
        oai_message = {k: message[k] for k in ("content", "function_call", "name", "context") if k in message}
        if "content" not in oai_message:
            if "function_call" in oai_message:
                span.set_attribute("is_function_call", True)
                oai_message["content"] = None  # if only function_call is provided, content will be set to None.
            else:
                span.set_attribute("is_function_call", False)
                return False
        ...
        span.set_attribute("message.oai_processed", json.dumps(oai_message))
        return True

    @tracer.start_as_current_span("ConversableAgent.send")
    def send(
        self: Self,
        ...
        """
        span = trace.get_current_span()
        span.set_attribute("recipient_agent.name", recipient.name)
        span.set_attribute("sent_message", json.dumps(message))
        span.set_attribute("self.agent.name", self.name)
        span.set_attribute("agent_type", self.__class__.__name__)
        # When the agent composes and sends the message, the role of the message is "assistant"
        # unless it's "function".
        valid = self._append_oai_message(message, "assistant", recipient) #FIXME: in main `conversation_id=recipient` which seems a bug

        if isinstance(message, dict):
            m = message.copy()
            m["recipient_agent"] = recipient.name
        else:
            m = message
        self._append_to_log(event="message_sent", message=m)
        if valid:
    ...
    @tracer.start_as_current_span("ConversableAgent.a_send")
    async def a_send(
        self: Self,
        message: Union[Dict, str],
        recipient: Agent,
        ...
        # When the agent composes and sends the message, the role of the message is "assistant"
        # unless it's "function".
        valid = self._append_oai_message(message, "assistant", self)
        if valid:
    ...
    @tracer.start_as_current_span("ConversableAgent._process_received_message")
    ...
    @tracer.start_as_current_span("ConversableAgent.receive")
    def receive(
        self,
        message: Union[Dict, str],
        sender: Agent,
        ...
        """
        span = trace.get_current_span()
        span.set_attribute("sender_agent.name", sender.name)
        span.set_attribute("received_message", json.dumps(message))
        span.set_attribute("self.agent.name", self.name)
        span.set_attribute("agent_type", self.__class__.__name__)
    ...
    @tracer.start_as_current_span("ConversableAgent.a_receive")
    async def a_receive(
        self,
        message: Union[Dict, str],
        sender: Agent,
    ...
    @tracer.start_as_current_span("ConversableAgent.initiate_chat")
    def initiate_chat(
        self,
        recipient: "ConversableAgent",
    ...
    @tracer.start_as_current_span("ConversableAgent.a_initiate_chat")
    ...
    @tracer.start_as_current_span("ConversableAgent.stop_reply_at_receive")
    ...
    @tracer.start_as_current_span("ConversableAgent.generate_oai_reply")
    def generate_oai_reply(
        ...
        try:
            ...
            logging.error("Error when calling OpenAIWrapper.create:", e)
            return False, f"error: {e}"

        span.set_attribute("response.type.openai.ChatCompletion", isinstance(response, ChatCompletion))
    ...
    def generate_code_execution_reply(
        ...
        if last_n_messages == "auto":
            ...
            for i in range(len(messages)):
                ...
                if "role" not in message:  # noqa: SIM114
                    break
        ...
        for i in range(min(len(messages), messages_to_scan)):
            ...
            if not message["content"]:
                span.add_event("no code block, `content` missing.")
                continue
            code_blocks = extract_code(message["content"])
            if len(code_blocks) == 1 and code_blocks[0][0] == UNKNOWN:
                span.add_event("no code block, in `content`")
                continue
    ...
    @tracer.start_as_current_span("ConversableAgent.generate_function_call_reply")
    ...
    @tracer.start_as_current_span("ConversableAgent.generate_async_function_call_reply")
    ...
    @tracer.start_as_current_span("ConversableAgent.check_termination_and_human_reply")
    ...
    @tracer.start_as_current_span("ConversableAgent.a_check_termination_and_human_reply")
    ...
    @tracer.start_as_current_span("ConversableAgent.generate_reply")
    def generate_reply(
        self,
        messages: Optional[List[Dict]] = None,
        sender: Optional[Agent] = None,
        ...
        span.set_attribute("message_history", json.dumps(messages))
        for reply_func_tuple in self._reply_func_list:
            ...
            if inspect.iscoroutinefunction(reply_func):
                continue
            sender_matches_trigger = self._match_trigger(reply_func_tuple["trigger"], sender)
            if sender_matches_trigger:
                print(
                    f"reply_func `{reply_func.__name__}` is a coroutine function: {inspect.iscoroutinefunction(reply_func)}"
                )
                final, reply = reply_func(self, messages=messages, sender=sender, config=reply_func_tuple["config"])
                span.add_event(
                    "reply_func.sender_match_trigger",
                    attributes={
                        "reply_func.name": reply_func.__name__,
                        "reply_func.is_coroutine": inspect.iscoroutinefunction(reply_func),
                        "reply_func.is_final": final,
                        "reply_func.reply_content": reply,
                    },
                )
                if final:
                    span.set_attribute("reply.is_default_reply", "False")
                    span.set_attribute("reply.content", reply)
                    return reply
        span.set_attribute("is_default_reply", "True")
        span.set_attribute("reply.content", json.dumps(self._default_auto_reply))
        return self._default_auto_reply

    @tracer.start_as_current_span("ConversableAgent.a_generate_reply")
    ...
    @tracer.start_as_current_span("ConversableAgent.get_human_input")
    ...
    @tracer.start_as_current_span("ConversableAgent.a_get_human_input")
    ...
    def run_code(self, code, **kwargs):
        ...
        """
        with tracer.start_as_current_span("ConversableAgent.run_code") as span:
            span.set_attribute("code", code)
            return execute_code(code, **kwargs)

    @tracer.start_as_current_span("ConversableAgent.execute_code_blocks")
    def execute_code_blocks(self, code_blocks):
        ...
        for i, code_block in enumerate(code_blocks):
            with tracer.start_as_current_span(f"executing code block {i}") as code_block_span:
                ...
                code_block_span.set_attribute("code_block.lang_was_inferred", bool(lang))
                if not lang:
                ...
                code_block_span.set_attributes({"code_block.lang": lang, "code_block.code": code})
                if lang in ["bash", "shell", "sh"]:
                    code_block_span.set_attribute("code_block.code_execution_config", self._code_execution_config)
                    exitcode, logs, image = self.run_code(code, lang=lang, **self._code_execution_config)
                elif lang in ["python", "Python"]:
                    if code.startswith("# filename: "):  # noqa: SIM108
                ...
                if image is not None:
                    code_block_span.add_event("executing code in docker container.")
                    self._code_execution_config["use_docker"] = image
                logs_all += "\n" + logs
                code_block_span.set_attributes(
                    {
                        "code_block.exitcode": exitcode,
                        "code_block.logs": logs_all,
                        "code_block.exec_in_container": bool(image),
                    }
                )
                if exitcode != 0:
    ...
    def _format_json_str(jstr):
        r"""Remove newlines outside of quotes, and handle JSON escape sequences.
    ...
    @tracer.start_as_current_span("ConversableAgent.execute_function")
    ...
    @tracer.start_as_current_span("ConversableAgent.a_execute_function")
    ...
    @tracer.start_as_current_span("ConversableAgent.generate_init_message")
    ...
    @tracer.start_as_current_span("ConversableAgent.register_function")
    ...
    @tracer.start_as_current_span("ConversableAgent.update_function_signature")
