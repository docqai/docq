...
def search_stage(
    ...
    if callable(reranker):
        ...
        if len(reranker_params) == 2:
            reranked_results = reranker(combined_results, processed_queries)
        elif len(reranker_params) == 1:
            reranked_results = reranker(combined_results)
        else:
